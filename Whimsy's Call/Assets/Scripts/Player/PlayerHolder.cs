...
public class PlayerHolder : MonoBehaviour
{
    ...
    public List<AttackObject> debugAttacks = new List<AttackObject>();
    ...
    void SendHold()
    {
        V2_moveInput += new Vector2(curClick.screenPos.x - initialClick.screenPos.x, curClick.screenPos.y - initialClick.screenPos.y)/tapInput.moveStickSize;
        V2_moveInput = Vector2.ClampMagnitude(V2_moveInput, 1);
    }
    ...
    void ApplyMovement(Vector2 moveDir)
    {
        ...
        forcePush *= physicsVar.moveMultiplier;

        if (playerMover.velocity.magnitude > physicsVar.maxSpeed && physicsVar.moveMultiplier <= 1)
    ...
    void PlayerModelRotation()
    {
        if (playerMoveDirection.magnitude > 0)
        {
            ...
            playerModel.rotation = Quaternion.RotateTowards(playerModel.rotation, Quaternion.LookRotation(playerMoveDirection),Time.deltaTime * 360 * physicsVar.rotMultiplier);
        }
    }
    ...
    {
        if (cooldowns.dodgeCool <= 0 && physicsVar.moveMultiplier <= 1)
        {
    ...
    void ActivateAbility (int abilNum)
    {
        ...
        {
            if (debugAttacks.Count > abilNum)
            {
                for (int i = 0; i < debugAttacks[abilNum].attacks.Count; i++)
                {
                    GameObject GO = Instantiate(debugAttacks[abilNum].attacks[i].prefab, playerMover.position,playerModel.rotation,transform);
                    AttackObject._attackObject temp = debugAttacks[abilNum].attacks[i];
                    if (temp.moveWithPlayer)
                        GO.transform.parent = playerModel.transform;
                    
                    temp.hitNonPlayerCreatures = temp.hitEnemies;
                    temp.hitPlayerCreatures = temp.hitFriendlies;
                    GO.GetComponent<Attack>().UpdateInfo(temp);
                }
                cooldowns.abilCool[abilNum] = debugAttacks[abilNum].cooldown;
                physicsVar.moveMultiplier = debugAttacks[abilNum].moveMultiplier;
                physicsVar.rotMultiplier = debugAttacks[abilNum].rotMultiplier;
                Invoke("ResetMoveMultiplier", debugAttacks[abilNum].multiplierDuration);
            }
        }
    }

    void ResetMoveMultiplier()
    {
        physicsVar.moveMultiplier = 1f;
        physicsVar.rotMultiplier = 1f;
    }
    ...
    public class _TapInput
    {
        ...
        [Tooltip("How big the movement stick is in pixels")]
        public float moveStickSize = 100f;
    }
